...
5. **History**: Last 3 user messages included as context, each labelled with the sender's display name (which jev can also say), and jev's own turn labelled with the bot's server nickname; jev's own broken output is excluded (it poisons follow-ups), but its emoji reactions are kept
