...
4. **Penalty system**: Content words penalized 2.5x per reuse, stopwords 1.6x — prevents "is are I is are" loops. Fillers and greetings that say the same thing (`SIMILAR`: yes/yeah/yea, hi/hey/hello, …) count as repeats of each other, so "Hi hey hi hey" is penalised like "Hi hi hi hi"; sound-alikes ("Google goo woo") are left alone. Jev also copies itself from one reply to the next ("Dunno? Dunno?" four times running), so each of its last 3 answers (`ECHO_TURNS`) that said a word divides it by 2 (`ECHO_PENALTY`), unless the message it's answering used that word too
